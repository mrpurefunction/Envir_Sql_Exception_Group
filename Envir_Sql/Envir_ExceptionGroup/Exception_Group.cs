...
namespace Envir_ExceptionGroup
{
    public class Exception_Group
    {
        ...
        /// <summary>
        /// DUST-special condition 20150730
        /// </summary>
        /// <param name="ts"></param>
        /// <param name="offsetahead"></param>
        /// <param name="offsetbehind"></param>
        /// <returns></returns>
        public DataSet GetSconRelatedDs_DUST(DateTime ts, int offsetahead, int offsetbehind, int machineid)
        {
            try
            {
                StringBuilder sb = new StringBuilder();
                sb.Append("select * from V_RuleResult_SpecialCon_DUST t where t.machineid = '" + machineid.ToString() + "' and ");
                sb.Append("t.timelog < '" + ts.AddHours(offsetbehind).ToString("yyyy-MM-dd HH:mm:ss") + "' and ");
                sb.Append("t.timelog > '" + ts.AddHours(-offsetahead).ToString("yyyy-MM-dd HH:mm:ss") + "'");
                Database db = DatabaseFactory.CreateDatabase("dbconn");
                System.Data.Common.DbCommand dbc = db.GetSqlStringCommand(sb.ToString());
        ...
        /// <param name="machineid"></param>
        /// <returns></returns>
        public int? GetSconRelatedCount_DUST(DateTime ts, int offsetahead, int offsetbehind, int machineid)
        {
            try
            {
                StringBuilder sb = new StringBuilder();
                sb.Append("select count(*) from V_RuleResult_SpecialCon_DUST t where t.machineid = '" + machineid.ToString() + "' and ");
                sb.Append("t.timelog < '" + ts.AddHours(offsetbehind).ToString("yyyy-MM-dd HH:mm:ss") + "' and ");
                sb.Append("t.timelog > '" + ts.AddHours(-offsetahead).ToString("yyyy-MM-dd HH:mm:ss") + "'");
                Database db = DatabaseFactory.CreateDatabase("dbconn");
                System.Data.Common.DbCommand dbc = db.GetSqlStringCommand(sb.ToString());
